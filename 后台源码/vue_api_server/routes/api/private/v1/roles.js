...
router.post("/",
	// 参数验证
	function(req,res,next) {
		if(!req.query.roleName) return res.sendResult(null,400,"角色名称不能为空");
		next();
	},
	// 处理业务逻辑
	function(req,res,next) {
...
router.post("/:id/rights",
	...
	// 业务逻辑
	function(req,res,next) {
		roleServ.updateRoleRight(req.params.id,req.query.rids,function(err,newRole){
			if(err) return res.sendResult(null,400,err);
...
// 删除角色权限
router.delete("/:roleId/rights/:rightId",
	// 参数验证
	function(req,res,next) {
	console.log(req.params)
	console.log(req.params.rightId)
