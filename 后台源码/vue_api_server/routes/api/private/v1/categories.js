...
router.post("/",
	...
	function(req,res,next) {
		catServ.addCategory({
			"cat_pid":req.query.cat_pid,
			"cat_name":req.query.cat_name,
			"cat_level":req.query.cat_level
		},function(err,result) {
...
router.post("/:id/attributes",
	...
	function(req,res,next) {
		...
		if(!req.query.attr_name) return res.sendResult(null,400,"参数名称不能为空");

		if(!req.query.attr_sel || (req.query.attr_sel != "only" && req.query.attr_sel != "many")) {
			return res.sendResult(null,400,"参数 attr_sel 类型必须为 only 或 many");
		}
		/*
		if(!req.query.attr_write || (req.query.attr_write != "manual" && req.query.attr_write != "list")) {
	...
	function(req,res,next) {
		...
		{
			"attr_name" : req.query.attr_name,
			"cat_id" : req.params.id,
			"attr_sel" : req.query.attr_sel,
			"attr_write" : req.query.attr_sel == "many" ? "list" : "manual",//req.query.attr_write,
			"attr_vals" : req.query.attr_vals ? req.query.attr_vals : ""
		},
		function(err,attr) {
			if(err) return res.sendResult(null,400,err);
...
router.put("/:id/attributes/:attrId",
	...
	console.log(req)
		...
		if(!req.query.attr_name || req.query.attr_name == "") return res.sendResult(null,400,"参数名称不能为空");
