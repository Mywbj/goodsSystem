...
router.post("/",
	...
	function(req,res,next) {
		...
		if(isNaN(parseInt(req.query.rid))) req.query.rid = -1;//return res.sendResult(null,200,"角色ID必须是数字");
		next();
	},
	// 处理业务逻辑
	function(req,res,next) {
...
router.put("/:id/role",
	...
	function(req,res,next) {
		...
		if(!req.query.rid) res.sendResult(null,400,"权限ID不能为空"); 
		next();
	},
	// 处理业务逻辑
	function(req,res,next) {
		mgrServ.setRole(req.params.id,req.query.rid,function(err,manager){
			if(err) return res.sendResult(null,400,err);
